...
const PAGE_SIZE = 10;
const USER_ACTIONS_KEY = "biology_bank_user_actions_v1";
...
async function loadData() {
  const [questionRes, topicRes] = await Promise.all([
    window.assetFetch("/data/biology/processed/questions.json"),
    window.assetFetch("/data/biology/topic-map.json"),
  ]);
...
function parsePaperMeta(paperLabel) {
  const m = String(paperLabel || "").match(
    /^(May|November)\s+(\d{4})(?:\s+Biology)?\s+Paper\s+([123](?:[AB])?)(?:\s+(TZ\d|NTZ))?\s+(HL|SL)$/i
  );
  if (!m) {
    return null;
  }
...
function buildQuestionNode(q) {
  ...
  if (qImages.length > 0) {
    ...
    questionTextEl.textContent = q.question_text ? q.question_text : "Question screenshot is being prepared.";
...
function openCompareModal(q) {
  ...
  if (qImages.length > 0) {
    ...
    p.textContent = q.question_text ? q.question_text : "Question screenshot is being prepared.";
...
function createImageWithFallback(relPath, altText) {
  ...
  img.alt = altText;
  img.loading = "lazy";
  img.src = window.assetUrl(`/data/biology/processed/${relPath}`);
  const legacyRelPath = legacyImageRelPath(relPath);
  if (legacyRelPath !== relPath) {
    img.addEventListener(
      "error",
      () => {
        img.src = window.assetUrl(`/data/biology/processed/${legacyRelPath}`);
      },
      { once: true }
    );
  }
