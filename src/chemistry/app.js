...
async function loadData() {
  const [questionRes, topicRes] = await Promise.all([
    window.assetFetch("/data/chemistry/processed/questions.json"),
    window.assetFetch("/data/chemistry/topic-map.json"),
  ]);
...
function createImageWithFallback(relPath, altText) {
  ...
  img.alt = altText;
  img.loading = "lazy";
  img.src = window.assetUrl(`/data/chemistry/processed/${relPath}`);
  const legacyRelPath = legacyImageRelPath(relPath);
  if (legacyRelPath !== relPath) {
    img.addEventListener(
      "error",
      () => {
        img.src = window.assetUrl(`/data/chemistry/processed/${legacyRelPath}`);
      },
      { once: true }
    );
  }
