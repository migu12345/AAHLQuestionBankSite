...
async function loadData() {
  const [questionRes, topicRes] = await Promise.all([
    window.assetFetch("/data/economics/processed/questions.json"),
    window.assetFetch("/data/economics/topic-map.json"),
  ]);
...
function createImageWithFallback(relPath, altText) {
  ...
  img.alt = altText;
  img.loading = "lazy";
  img.src = window.assetUrl(`/data/economics/processed/${relPath}`);
  const legacyRelPath = legacyImageRelPath(relPath);
  if (legacyRelPath !== relPath) {
    img.addEventListener(
      "error",
      () => {
        img.src = window.assetUrl(`/data/economics/processed/${legacyRelPath}`);
      },
      { once: true }
    );
  }
...
function renderQuestions(reset = true) {
  if (reset) {
    ...
    if (state.filteredQuestions.length === 0) {
      ...
      questionList.innerHTML = "<p>No questions yet — data coming soon.</p>";
